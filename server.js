...
/* ---------------- SPA fallbacks ---------------- */
app.get('/payment-return', /* -------- Saved cards (Flutterwave-ready) -------- */
async function ensureCardsTable(){
  ...
  await pool.query(`
    create table if not exists saved_cards (
      ...
      nickname text, flw_token text,
      created_at timestamptz default now()
    );
    create index if not exists saved_cards_user_idx on saved_cards(user_phone, created_at desc);
  `).catch(()=>{});
}
ensureCardsTable();

app.get('/api/payments/cards', auth, async (req, res) => {
  if (pool){
    const rows = await q('select id,brand,last4,exp_month,exp_year,nickname,created_at from saved_cards where user_phone=$1 order by created_at desc', [req.user.phone]);
    return res.json({ cards: rows });
  }
  res.json({ cards: [] });
});

app.post('/api/payments/cards', auth, async (req, res) => {
  const { brand, last4, expMonth, expYear, nickname, token } = req.body || {};
  if (!brand || !last4) return res.status(400).json({ error: 'Missing card info' });
  if (pool){
    const r = await q(
      `insert into saved_cards(user_phone,brand,last4,exp_month,exp_year,nickname,flw_token)
       values($1,$2,$3,$4,$5,$6,$7) returning id,brand,last4,exp_month,exp_year,nickname`,
      [req.user.phone, brand, last4, expMonth||null, expYear||null, nickname||null, token||null]
    );
    return res.json({ card: r[0] });
  }
  res.json({ card: { id: crypto.randomUUID(), brand, last4, expMonth, expYear, nickname } });
});

app.delete('/api/payments/cards/:id', auth, async (req, res) => {
  if (pool){
    await q('delete from saved_cards where id=$1 and user_phone=$2', [req.params.id, req.user.phone]);
  }
  res.json({ ok: true });
}); app.get('/api/payments/history', auth, async (req, res) => {
  if (pool) return res.json(await q('select * from orders where customer_phone=$1 order by created_at desc limit 50', [req.user.phone]));
  res.json([...mem.orders.values()].filter(o => o.customer_phone === req.user.phone));
}); (req, res) => res.sendFile(path.join(__dirname, 'public', 'payment-return.html')));
app.get('/portal', (req, res) => res.sendFile(path.join(__dirname, 'public', 'portal.html')));
app.get('/portal.html', app.get('/scan',     (req, res) => res.sendFile(path.join(__dirname, 'public', 'scan.html')));
app.get('/scan.html',(req, res) => res.sendFile(path.join(__dirname, 'public', 'scan.html'))); (req, res) => res.sendFile(path.join(__dirname, 'public', 'portal.html')));
